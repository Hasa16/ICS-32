...
class GameState:
    ...
    def tick(self) -> bool:
        ...
        if self._faller.active:
            if self._faller.state == _FALLER_STOPPED:
                ...
                if self._faller.state == _FALLER_STOPPED:
                    value = False
                    if self._faller.get_rows() - 2 < 0:
                        value = True
                    for i in range(3):
                        self.board_rows[self._faller.get_rows() - i][self._faller.get_column()] \
                            = self._faller.contents[i]
                        self.board_states[self._faller.get_rows() - i][self._faller.get_column()] = OCCUPIED
    ...
    def spawn_faller(self, column: int, faller: [str, str, str]) -> None:
        ...
        self.board_rows[0][self._faller.get_column()] = self._faller.contents[0]
        self.board_states[0][self._faller.get_column()] = CELL_FALLER_MOVING
        self._update_faller_state()
    ...
    def move_faller(self, direction: int) -> None:
        ...
        if not direction == RIGHT and not direction == LEFT:
            return
        if (direction == LEFT and self._faller.get_column() == 0) or (
                direction == RIGHT and self._faller.get_column() == self.columns - 1):
            return
        target_column = self._faller.get_column() + direction
        for i in range(3):
            if self._faller.get_rows() - i < 0:
                break
            if self.board_states[self._faller.get_rows() - i][target_column] == OCCUPIED:
                return
        for i in range(3):
            if self._faller.get_rows() - i < 0:
                break
            self.move_cell(self._faller.get_rows() - i, self._faller.get_column(), direction)
        self._faller.set_col(target_column)
        self._update_faller_state()
    ...
    def rotate_faller(self) -> None:
        ...
        for i in range(3):
            self.board_rows[self._faller.get_rows() - i][self._faller.get_column()] = self._faller.contents[i]
        self._update_faller_state()
    ...
    def move_faller_down(self) -> None:
        ...
        Returns:
            None
        """
        if self._check_solid(self._faller.get_rows() + 1, self._faller.get_column()):
            return
        self.move_cell(self._faller.get_rows(), self._faller.get_column(), _DOWN)
        if self._faller.get_rows() - 1 >= 0:
            self.move_cell(self._faller.get_rows() - 1, self._faller.get_column(), _DOWN)
            if self._faller.get_rows() - 2 >= 0:
                self.move_cell(self._faller.get_rows() - 2, self._faller.get_column(), _DOWN)
            else:
                self.board_rows[self._faller.get_rows() - 1][self._faller.get_column()] = self._faller.contents[2]
                self.board_states[self._faller.get_rows() - 1][self._faller.get_column()] = CELL_FALLER_MOVING
        else:
            self.board_rows[self._faller.get_rows()][self._faller.get_column()] = self._faller.contents[1]
            self.board_states[self._faller.get_rows()][self._faller.get_column()] = CELL_FALLER_MOVING
        self._faller.set_row(self._faller.get_rows() + 1)
    ...
    def _update_faller_state(self) -> None:
        ...
        Returns:
            None
        """
        target_row = self._faller.get_rows() + 1
        if self._check_solid(target_row, self._faller.get_column()):
        ...
        for i in range(3):
            row = self._faller.get_rows() - i
            if row < 0:
                return
            self.board_rows[row][self._faller.get_column()] = self._faller.contents[i]
            self.board_states[row][self._faller.get_column()] = state
